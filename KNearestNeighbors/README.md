...
```
You generally don't want to train a classifier on all your data because if we train on all of our data, then we have to test on the same data so we won't be able to tell how accurate our classifier is on data it hasn't seen before. We use a "train-test-split" to split our data into a training set and a testing set.   
